...
#'
CMetabLagrangeExtractor <- R6Class(
   classname = "CMetabLagrangeExtractor",
   inherit = disco::TransferFunctionSummarizer,
   public = list(
      ...
      #'
      #' @param signalIn
      #'   The input signal for the transfer function being summarized
      #' @param signalOut
      #'   The output signal for the transfer function being summarized
      #' @param outputPath
      ...
      )
      {
         ...
         attributes(self$table$time)$tzone <-
            attributes(results$downstreamTime)$tzone;
         self$table$time[self$index] <- results$downstreamTime[1];
